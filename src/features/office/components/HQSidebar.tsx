...
const TAB_KEYS: Record<HQSidebarTab, string> = {
  inbox: "hq.sidebar.tabInbox",
    ...
    <aside className="pointer-events-none fixed inset-y-0 right-0 z-20 flex justify-end">
      <div className="pointer-events-auto mt-14 flex shrink-0 flex-col items-end gap-1.5">
        <button
          ...
          aria-expanded={open}
          aria-label={open ? t("hq.sidebar.collapseAriaLabel") : t("hq.sidebar.openAriaLabel")}
        >
          <span className="block leading-none [writing-mode:vertical-rl]">
            {open ? t("hq.sidebar.collapseHQ") : t("hq.sidebar.openHQ")}
          </span>
        </button>
        ...
        <button
          ...
          aria-label={t("hq.sidebar.openMarketplaceAriaLabel")}
        >
          <span className="block leading-none [writing-mode:vertical-rl]">
            {t("hq.sidebar.marketplace")}
          </span>
        </button>
        ...
        <button
          ...
          aria-label={t("hq.sidebar.openAnalyticsAriaLabel")}
        >
          <span className="block leading-none [writing-mode:vertical-rl]">
            {t("hq.sidebar.analytics")}
          </span>
        </button>
      </div>
        ...
        >
          <div className="border-b border-cyan-500/15 px-4 py-3">
            <div className="font-mono text-[10px] font-semibold tracking-[0.32em] text-cyan-300/80">
              {analyticsOnly ? t("hq.sidebar.analyticsTitle") : t("hq.sidebar.headquartersTitle")}
            </div>
            <div className="mt-1 font-mono text-[11px] text-white/45">
              {analyticsOnly
                ? t("hq.sidebar.analyticsDesc")
                : t("hq.sidebar.headquartersDesc")}
            </div>
            {!railOnly && onAddAgent ? (
              <button
                type="button"
