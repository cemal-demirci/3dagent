...
const STORE_DIR = path.join("3dagent", "task-manager");
...
const normalizeTaskRecord = (value: unknown): SharedTaskRecord | null => {
  ...
  return {
    ...
    source: isTaskBoardSource(record.source) ? record.source : "3dagent_manual",
...
export const upsertSharedTask = (
  ...
  const rawSource = (task.source as TaskBoardSource | undefined) ?? existing?.source ?? "3dagent_manual";
  ...
  const next: SharedTaskRecord = {
    ...
    source: isTaskBoardSource(rawSource) ? rawSource : "3dagent_manual",
