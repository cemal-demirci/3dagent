...
This repository ships developer-created marketplace skills as packaged assets that 3DAgent can install into an OpenClaw workspace through the gateway.
...
- Every skill must define a `## Trigger` section that explains what activates the skill and what the agent should physically do in 3DAgent when it activates.
...
- The JSON block is what 3DAgent parses at runtime.
...
- If a skill has no trigger block, 3DAgent can fall back to the central default trigger registry in `src/lib/office/places.ts`.

## 2. Mirror the files into `src/lib/skills/packaged.ts`

3DAgent installs packaged skills from client-safe embedded strings, not by reading `assets/skills/...` directly at runtime.
